...
while [ ${m} -lt 11 ]; do
	...
	cp my_control.usertree ${mpestPATH}/my_control.usertree_${basic_index}
	echo "  copying my_control.usertree_${basic_index} "
...
start=`date +%s`
    ...
    perl extract_subsets.pl -i ds_${index}
    ...
    do
        echo "          Processing $f"
        python induced_subtree_from_taxa.py gene_tree_${r}_src ${f}
